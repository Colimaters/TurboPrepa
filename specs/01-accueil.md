...
- Affiche une **citation réelle et attribuée** (auteur, philosophe, figure historique,
  militaire ou politique), pas une phrase inventée par l'IA.
...
- Une citation différente chaque jour, non répétée avant d'avoir épuisé le stock disponible
  ...
  rapprochées).
- Prévoir un stock initial d'au moins 30 citations vérifiables (thèmes cohérents avec la
  préparation d'un concours : courage, discipline, droit, justice, persévérance, service
  public — inclure par exemple une devise institutionnelle comme celle de la Gendarmerie).
- Le stock doit être **éditable** : structure de données simple (liste d'objets
  `{ texte, auteur, source?, attributionIncertaine: bool }`) pour permettre à l'utilisateur
  d'en ajouter/retirer facilement plus tard.
- Option future (non bloquante pour la V1) : tri par thème (droit, courage, discipline…).
